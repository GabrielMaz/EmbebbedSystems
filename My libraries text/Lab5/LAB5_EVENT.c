...
void getEventName(char *name) {
    ...
    CLEAR_BUFFER();
    while(!sock_gets(&socket, name, 11)) { OSTimeDlyHMSM (0, 0, 0, BASIC_DELAY); }
...
void getEventLeds(char* leds) {
    ...
    while(!(sock_gets(&socket, leds, 9))) { OSTimeDlyHMSM (0, 0, 0, BASIC_DELAY); }
...
void deleteEventEthernetUI() {
    ...
    while(!sock_gets(&socket, data, 4)) { OSTimeDlyHMSM (0, 0, 0, BASIC_DELAY); }
